...
This document defines the shared objective, system model, test flows, and success criteria so we stay aligned. It extends the single-user scope in [PHASE1_DESIGN.md](PHASE1_DESIGN.md) to **two users** and **state-transition validation**.
...
See [API_REFERENCE_FROM_LOAD_TESTS.md](API_REFERENCE_FROM_LOAD_TESTS.md) for env mapping and [NOTION_TASKS.md](NOTION_TASKS.md) for config tasks.
...
|---------------------------|-------------------------------|
| [PHASE1_DESIGN.md](PHASE1_DESIGN.md) | Phase 1 is single-user flow; this doc adds **two users**, matching, and full state-validation scope. |
| [FRAMEWORK.md](FRAMEWORK.md)         | Stack (TestNG, RestAssured, Service Object, POJOs, Filters, GitHub Actions) and execution flow. |
...
| [API_REFERENCE_FROM_LOAD_TESTS.md](API_REFERENCE_FROM_LOAD_TESTS.md) | Endpoints, request/response shapes, EIP-712/1193, sig-server options. |

When implementing: use **OBJECTIVES_AND_ALIGNMENT** for what to validate; use **API_REFERENCE** and **FRAMEWORK** for how to implement; use **NOTION_TASKS** to track work.
