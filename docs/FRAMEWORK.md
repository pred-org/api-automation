...
- Use JDBC to fetch data, map to POJO, pass into API request (e.g. for data-driven or DB-backed assertions). Prerequisites (connection, tables, mapping, assertion rules) should be agreed with the team and tracked in `docs/GAPS-AND-ROADMAP.md` before implementation.
...
## 15. Design annex: enums, POJOs, utils, assertions, database

Merged from the former `DESIGN_DISCUSSION_ENUMS_POJOS_UTILS_ASSERTIONS_DB.md`. No code here; placement and conventions so implementations stay aligned.

### 15.1 Enums

**Where:** Model layer: `src/main/java/com/pred/apitests/model/` or `model/enums/`. Enums capture closed sets used in requests or assertions.

**What to put in enums:** Fixed values in requests or response checks (e.g. order side LONG/SHORT, order type LIMIT/MARKET, chain type). Do not put open-ended IDs, amounts, or timestamps in enums.

### 15.2 POJOs

**Where:** `model/request/` and `model/response/`.

**Request POJOs:** Shape outgoing bodies; use Builder where helpful. Map 1:1 to API contract.

**Response POJOs:** Parse JSON for type-safe assertions and passing data to later steps. Add where justified; `jsonPath()` is fine for simple cases.

### 15.3 Utils

**Where:** `src/main/java/.../util/` for shared main code; `src/test/java/.../util/` for test-only helpers.

**Typical utils:** timestamps/nonce, JSON helpers, poll/retry until condition, env fallbacks. Keep API-shaped logic in services, not generic utils.

### 15.4 Assertions

**Where:** Only in `*Test.java` methods. Services return `Response` or POJOs; they do not assert.

**Libraries:** RestAssured for status and simple body paths; AssertJ for extracted values and balance/position logic. Per **OBJECTIVES_AND_ALIGNMENT.md**, derive expected balances from rules and prior state; do not hardcode absolute balances.

### 15.5 Database (future)

Not in scope for current automation. When needed: JDBC or repository helpers in a dedicated package (e.g. `com.pred.apitests.db`); tests compare API vs DB. Prerequisites (connection, tables, mapping, rules) should be agreed and tracked in **GAPS-AND-ROADMAP.md** before implementation.
